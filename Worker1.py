...
class Worker:
    def __init__(self, id=0, name="", surname="", department="", salary=0):
        self.id = id
        self.name = name
    ...
    def worker_db(self, filename):
        with open(filename, "r", newline="") as file:
            ...
            for row in reader:
                ...
                worker = Worker(id, name, surname, department, salary)
                self.workers.append(worker)
    ...
    def edit_worker(self, id):
        for worker in self.workers:
            if worker.get_id() == id:
                while True:
                    data = input(
                        "Enter the data for changes (id, name, surname, department, salary)(Press 0 to return to the menu): "
                    )
                    if data == "id":
                        new_id = input("New ID: ")
                        worker.set_id(new_id)
                    elif data == "name":
...
def main():
    ...
    collection.worker_db("workers.csv")
    while True:
        print("\nChoose option: ")
        print("1.Add worker")
        print("2.Show all")
